...
async def test_analyze_tender_success():
    ...
    # Mock DB
    mock_db = MagicMock()
    mock_db.get_tender = AsyncMock(
        return_value={"headline": "Test Tender", "description": "We need Machine Learning services."}
    )
    mock_db.get_all_keywords = AsyncMock(return_value=[MagicMock(term="Existing")])

    # Mock OpenAI
    mock_openai = MagicMock()
    mock_openai.generate_text = MagicMock(
        return_value='[{"term": "Machine Learning", "weight": 1.5, "type": "Service"}]'
    )

    # Initialize Service manually with mocks patched
    with (
        patch("domains.rating.analysis_service.db", mock_db),
