...
async def seed_test_keywords():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10.0) as client:
        ...
        test_keywords = [
            ...
            {
                "term": f"legacy system-{uuid.uuid4().hex[:6]}",
                "category": "architecture",
                "weight": -2.0,
                "type": "Exclusion",
            },
            {
                "term": f"electric mobility-{uuid.uuid4().hex[:6]}",
...
async def test_combined_search_and_category_filter(seed_test_keywords):
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10.0) as client:
        ...
        filtered = [k for k in all_keywords if "cl" in k["term"].lower() and k.get("category") == "technology"]
...
async def test_combined_search_and_weight_filter(seed_test_keywords):
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10.0) as client:
        ...
        filtered = [k for k in all_keywords if "i" in k["term"].lower() and 3.0 <= k["weight"] <= 5.0]
