...
async def get_keyword_tree():
    ...
    result = {type_name: sorted(list(subtypes)) for type_name, subtypes in tree.items()}
...
async def get_score_distribution(
    ...
    return await db.get_score_distribution(
        search_text=search, website=website, keyword=keyword, rating_category=rating_category
    )
...
async def import_keywords_file(
    ...
    delete_missing: bool = Query(
        False, description="If true, deletes keywords in DB that are missing from YAML (Sync mode)."
    ),
):
    """
    ...
    try:
        ...
        for up_kw in uploaded_keywords:
            ...
            if term_key not in current_map:
                ...
                if (
                    existing.weight != up_kw.weight
                    or existing.type != up_kw.type
                    or existing.sub_type != up_kw.sub_type
                    or existing.sub_category != up_kw.sub_category
                    or existing.category != up_kw.category
                ):
        ...
        summary = KeywordImportSummary(
            created=created, updated=updated, deleted=deleted, total_count=len(uploaded_keywords)
        )

        if dry_run:
            return KeywordImportResult(
                summary=summary, dry_run=True, success=True, message="Dry run successful. Review changes."
            )
